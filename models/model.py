...
class CustomModel(TorchModelV2, nn.Module):
    ...
    def get_curiosity_loss(self, obs_batch, next_obs_batch, actions):
        """Compute ICM loss and intrinsic reward."""
        if not model_config.get("custom_model_config", {}).get("use_curiosity", False):
            return 0.0, 0.0

        # Get feature representations
        current_features = self.feature_net(obs_batch)
        next_features = self.feature_net(next_obs_batch)

        # Forward dynamics loss
        action_type = actions[:, :4]  # First 4 elements are action logits
        action_params = actions[:, 4:]  # Rest are continuous parameters
        pred_next_features = self.forward_net(torch.cat([current_features, action_type, action_params], dim=1))
        forward_loss = F.mse_loss(pred_next_features, next_features.detach())

        # Inverse dynamics loss
        pred_actions = self.inverse_net(torch.cat([current_features, next_features], dim=1))
        inverse_loss = F.mse_loss(pred_actions, actions)

        # Total loss
        total_loss = self.forward_loss_weight * forward_loss + (1 - self.forward_loss_weight) * inverse_loss

        # Intrinsic reward is prediction error in feature space
        intrinsic_reward = self.curiosity_weight * F.mse_loss(pred_next_features, next_features, reduction='none').mean(dim=1)
